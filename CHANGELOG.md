...
- Member `and_then` accepts `just` results, including choices and `just<void>`. Choice branches may return different `just` types: their payloads form a normalized choice, with returned choices contributing their alternatives and `just<void>` contributing `pack<>`. If the result types agree after cv/ref removal, the result keeps that `just` type. Constructors also widen a `just<T>` into a choice containing `T`, or `just<void>` into one containing `pack<>`, unless the source carrier itself matches an alternative.
