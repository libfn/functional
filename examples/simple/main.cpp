...
TEST_CASE("Demo choice and graded monad", "[choice][and_then][inspect][transform][graded]")
{
  ...
  CHECK(parse("'abc'") == fn::as_choice(std::string_view{"abc"}));
  CHECK(parse(R"("def")") == fn::as_choice(std::string_view{"def"}));
  CHECK(parse("null") == fn::as_choice(nullptr));
  CHECK(parse("") == fn::as_choice(nullptr));
  CHECK(parse("true") == fn::as_choice(true));
  CHECK(parse("false") == fn::as_choice(false));
  CHECK(parse("1025") == fn::as_choice(1025l));
  CHECK(parse("10.25") == fn::as_choice(10.25));
  CHECK(parse("2e9") == fn::as_choice(2e9));
  CHECK(parse("5e9") == fn::as_choice(5e9));
  ...
  CHECK(a == fn::as_choice(true));
  CHECK(fn("123") == fn::choice<int>{123});
  CHECK(fn("2e9") == fn::choice<int>{2000000000});
  CHECK(fn("5e9") == fn::as_choice(5e9));
  CHECK(fn("") == fn::as_choice(nullptr));
  CHECK(fn("foo") == fn::as_choice(nullptr));
  ...
  static constexpr auto convert = []<typename T>(std::in_place_type_t<T>, std::string_view v) {
    return parse(v).apply([](auto &&v) -> fn::expected<T, InputError> {
      if constexpr (std::is_same_v<std::decay_t<decltype(v)>, T>) {
        ...
        return ::fn::unexpected<InputError>{InvalidType};
    });
  };
  ...
  static constexpr auto fn2 = [](std::string_view configuration, std::string_view hostname, std::string_view port,
    ...
    return (fn::expected<void, fn::copack<>>() //
           ...
           | fn::and_then([](auto const &config) -> fn::expected<int, NetworkError> {
               if (config.port < 1024)
                 return ::fn::unexpected<NetworkError>(ConnectError);
