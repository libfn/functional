  ...
  static auto make(parameters const &p) -> fn::expected<inputs, error>
  {
    ...
    static constexpr auto open_file
      ...
      if (auto const type = std::filesystem::status(path, ec).type();
        ...
        return ::fn::unexpected(file_not_found{{.path = std::move(path), .ec = ec}});
      }
      if (ec == std::errc::permission_denied) {
        return ::fn::unexpected(permission_denied{{.path = std::move(path), .ec = ec}});
      }
      return ::fn::unexpected(
...
constexpr inline struct algorithm_t {
  auto operator()(inputs const &inputs) const -> fn::expected<int, read_error>
  {
    ...
    for (auto const &source : inputs.streams) {
      ...
      return ::fn::unexpected(read_error{{.path = source.path, .ec = std::make_error_code(std::io_errc::stream)}});
    }
